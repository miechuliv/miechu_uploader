...
class ControllerCommonFileManager extends Controller {
    ...
    public function search() {
        ...
        if ($files) {
            foreach ($files as $file) {
                ...
                $diff = abs(strlen($this->request->get['name']) - strlen(basename($file)));



                // sprawdzamy czy pasuje do wyszukiwanej nazwy
                if(($leven = levenshtein($t(basename(strtolower($file))),strtolower($this->request->get['name']))) > 5)
                {
                    continue;
                }
    ...
    public function massupload()
    {
        ...
        for($i=0;$i<count($files['name']);$i++){
            ...
            if ($files['tmp_name'][$i]) {
                $filename = basename(html_entity_decode($files['name'][$i], ENT_QUOTES, 'UTF-8'));
                ...
                if ($files['error'][$i] != UPLOAD_ERR_OK) {
                    $json['error'] = 'error_upload_' . $files['error'][$i];
                }
            } else {
